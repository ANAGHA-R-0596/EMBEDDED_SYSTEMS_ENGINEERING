/*
 * DELAY_1SEC_NORMAL.c
 * TO BLINK AN LED WITH A DELAY OF 1 SECOND.
 * Created: 19-09-2025 12:39:30
 * Author : ANAGHA R
 */ 
 

#define F_CPU 16000000UL
#include <avr/io.h>

void delay_1s(void)
{
	unsigned int i, count = 62500;
	for(i = 0;i<count;i++) //This will run for 62500 for getting 1sec delay (1 sec = 62500 times)
	{
		TCNT0 = 0;
		TCCR0A = 0; // FOR NORMAL CLOCK SOURCE
		TCCR0B |= (1 << CS00); //001 FOR NO PRESCALING,SO SET CS00 TO 1.
		TIFR0 |= (1 << TOV0);  //SET OVERFLOW FLAG 0 BY WRITING 1 TO IT.
		while (!(TIFR0 & (1 << TOV0))); //WAITING TIMER OVERFLOW FLAG TO SET SO THAT WE GET 16US DELAY.
	}
}



int main(void)
{
	
    DDRB |= (1 << DDB5);
	
    while (1) 
    {
		PORTB ^= (1 << PORTB5);  // FOR BLINKING LED.
		delay_1s();
		
    }
}
	
	
	

